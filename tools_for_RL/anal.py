...
sys.path.append('/workspace/linjh/CoT_Factory/src')
from nlp_tools import Viz, richf
...
def anal_env(config, train_eval, display_id=100, n_sample_game=None):
    ...
    for game_file in game_files:
        with open(game_file, 'r') as f:
            data = json.load(f)
        task_type_list.append(data['task_type'])
    # use rich to show
    Viz.visualize_list(task_type_list, 'Task Type', 'task_type_list.csv')
    
    # game_files格式：xxx/trial_yy/zz.json
    # 统计xxx个数，也就是父目录的父目录
    print('以父类而不是以json文件为单位统计')
    task_type_list = []
    grand_parent_dir_list = set()
    for game_file in game_files:
        grand_parent_dir = os.path.dirname(os.path.dirname(game_file))
        if grand_parent_dir not in grand_parent_dir_list:
            grand_parent_dir_list.add(grand_parent_dir)
        else:
            continue
        with open(game_file, 'r') as f:
            data = json.load(f)
...
def main():
    ...
    richf('Analizing train set...')
    anal_env(config, 'train')
    
    richf('Analizing eval_in_distribution...')
    anal_env(config, 'eval_in_distribution')
    
    richf('Analizing eval_out_of_distribution...')
    anal_env(config, 'eval_out_of_distribution')
